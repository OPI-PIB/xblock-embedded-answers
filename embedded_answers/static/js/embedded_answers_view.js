...
function EmbeddedAnswersXBlockInitView(runtime, element) {
    const self = this;
    ...
    this.initEvents = () => {
        this.submitButton.click(function(eventObject) {
            ...
            var complete = true;
            var counter = 1;
            self.questionPromptContainer.find("input").each(function() {
                if (this.getAttribute('xblock_id') == self.xblock_id) {
                    if (this.value.length == 0) {
                        complete = false;
                        self.showNotification({ status: "error", msg: embedded_answersi18n.gettext('You haven\'t completed the question.')});
                    }
    ...
    this.preSubmit = () => {
        this.problemProgressContainer.text(embedded_answersi18n.gettext('(Loading...)'));
        if (this.prompt == '') {
            this.prompt = this.questionPromptContainer.html();
        }
    }
	...
	this.postSubmit = (result) => {
        ...
        this.restoreInputTexts(result.submissions);
        this.showSubmitFeedback(result.submit_feedback);
        this.showNotification(result.submit_notification);
        this.addDecorations(result.correctness, result.input_text_order);
	}
    ...
    this.showCorrectAnswer = (result) => {
        ...
        if (!this.questionPromptContainer.find('span').length){
            var i = 1;
            self.questionPromptContainer.find("input").each(function() {
                $('<span class="embedded_answers feedback_number_correct correct_answer">(' + i + ')</span>').insertAfter(this);
                i += 1;
            })
        }
	...
	this.restoreState = (result) => {
        if (result.completed == true || result.saved) {
            this.restoreInputTexts(result.input_texts);
        }
